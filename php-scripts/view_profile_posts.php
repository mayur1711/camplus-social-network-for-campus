...
$total = mysqli_num_rows(mysqli_query($connect, "SELECT post_id from post WHERE user_id = $user_Id"));
...
if($page<=$page_limit){
    ...
    $start = ($page - 1) * $limit;
  	...
  	$statement = mysqli_prepare($connect, "SELECT post.*, user.name, user.year from post
        INNER JOIN user ON post.user_id = user.user_id WHERE post.user_id = $user_Id
        ORDER BY post_id ASC limit $start, $limit");
